...
app.get("/send-payslips", async (req, res) => {
  try {
    ...
    for (const emp of employees) {
      try {
        const pdfBuffer = await generatePayslip(emp);
        ...
        await transporter.sendMail({
          ...
          attachments: [{
            filename: `${emp.employeeId}_${emp.name.replace(/\s+/g, "_")}_Payslip.pdf`,
            content: pdfBuffer
          }]
        });

        console.log(`✅ Sent payslip to ${emp.email}`);
      } catch (innerErr) {
        console.error(`❌ Failed for ${emp.email}:`, innerErr);
      }
    }

    res.send("Payslips sent successfully!");
  } catch (err) {
